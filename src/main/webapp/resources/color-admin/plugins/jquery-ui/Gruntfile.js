...
var
	...
	cssFiles = [
		...
		return "themes/js/jquery.ui." + component + ".css";
	}),
...
cssFiles.forEach(function( file ) {
	minifyCSS[ file ] = {
		...
		src: file,
		dest: "dist/" + file.replace( /\.css$/, ".min.css" ).replace( /themes\/base\//, "themes/js/minified/" )
	};
});
	...
	copy: {
		dist: {
			...
			renames: {
				...
				"dist/jquery-ui.css": "themes/js/jquery-ui.css",
				"dist/jquery-ui.min.css": "themes/js/minified/jquery-ui.min.css"
			},
		...
		dist_css_min: {
			src: "dist/themes/js/minified/*.css",
			strip: /^dist/,
			dest: "dist/<%= files.dist %>"
		},
		dist_units_images: {
		...
		dist_min_images: {
			src: "themes/js/img/*",
			strip: /^themes\/base\//,
			dest: "dist/<%= files.dist %>/themes/js/minified"
		},
		cdn: {
			src: [
				"AUTHORS.txt",
